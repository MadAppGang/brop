...
function createExtensionZip(useTimestamp = true) {
  ...
  const zipFileName = useTimestamp ? `brop-extension-${timestamp}.zip` : 'brop-extension.zip';
    ...
    if (fs.existsSync(manifestPath)) {
      try {
        ...
        if (manifest.background?.service_worker) {
          const bgScript = path.join(tempDir, manifest.background.service_worker);
          if (fs.existsSync(bgScript)) {
            console.log(`   ✅ Background script: ${manifest.background.service_worker}`);
          } else {
            console.log(`   ⚠️  Background script missing: ${manifest.background.service_worker}`);
          }
        }
...
function showHelp() {
  ...
  console.log('');
  console.log('Options:');
  console.log('  --no-timestamp    Create brop-extension.zip without timestamp');
  console.log('');
  console.log('This script packages the Chrome extension files into a zip archive');
