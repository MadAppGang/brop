...
class CDPRelayTester {
  ...
  async setup() {
    ...
    this.proxyWS = new WebSocket('ws://localhost:9222/devtools/browser/brop-bridge-comprehensive-test');
  ...
  async runTests() {
    ...
    const tests = [
      ...
      { 
        name: 'Create Target', 
        command: { id: 3, method: 'Target.createTarget', params: { url: 'about:blank' } } 
      },
      { 
        name: 'Runtime Evaluation', 
        command: { id: 4, method: 'Runtime.evaluate', params: { expression: '2 + 2' } } 
      },
      { 
        name: 'Page Navigation', 
        command: { id: 5, method: 'Page.navigate', params: { url: 'https://example.com' } } 
      },
      { 
        name: 'DOM Query', 
