	...
	try {
		...
		for (const test of bropTests) {
			...
			if (response.success === false) {
				...
				if (
					test.name.includes("Extension Version") &&
					response.result?.result
				) {
					console.log(
						`   Version: ${response.result.result.extension_version}`,
					);
				}
			}
		}
		...
		for (const test of cdpTests) {
			...
			if (response.error) {
				console.log(
					`❌ ${test.name}: ${response.error.message || "Unknown error"}`,
				);
			} else {
				console.log(`✅ ${test.name}: Success`);
