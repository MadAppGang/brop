...
class UnifiedBridgeTest {
  ...
  async testBropConnection() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.bropWs.on('error', (error) => {
        ...
        this.testResults.push({ test: 'BROP Connection', status: 'FAIL', error: error.message });
        reject(error);
      });
  ...
  async testCdpConnection() {
    ...
    return new Promise((resolve, reject) => {
      this.cdpWs = new WebSocket('ws://localhost:9222/devtools/browser/test-browser-session');
      ...
      this.cdpWs.on('error', (error) => {
        ...
        this.testResults.push({ test: 'CDP Connection', status: 'FAIL', error: error.message });
        reject(error);
      });
  ...
  async testCdpDiscovery() {
    ...
    try {
      ...
      if (versionData.Browser && versionData['Protocol-Version']) {
        ...
        this.testResults.push({ test: 'CDP Discovery /json/version', status: 'PASS' });
      } else {
      ...
      if (Array.isArray(listData) && listData.length > 0) {
        ...
        this.testResults.push({ test: 'CDP Discovery /json/list', status: 'PASS' });
      } else {
      ...
      this.testResults.push({ test: 'CDP Discovery', status: 'FAIL', error: error.message });
      throw error;
    }
  }
  ...
  async testProtocolSeparation() {
    ...
    return new Promise((resolve, reject) => {
      if (!this.bropWs || !this.cdpWs) {
        ...
        this.testResults.push({ test: 'Protocol Separation', status: 'FAIL', error: error.message });
        reject(error);
        return;
      }
      ...
      const checkComplete = () => {
        if (bropResponseReceived && cdpResponseReceived) {
          ...
          this.testResults.push({ test: 'Protocol Separation', status: 'PASS' });
          resolve();
        }
      };
      ...
      setTimeout(() => {
        // Send BROP command
        this.bropWs.send(JSON.stringify({
          id: 'brop-test-1',
      ...
      setTimeout(() => {
        if (!bropResponseReceived || !cdpResponseReceived) {
          const error = new Error(`Protocol separation incomplete - BROP: ${bropResponseReceived}, CDP: ${cdpResponseReceived}`);
          this.testResults.push({ test: 'Protocol Separation', status: 'FAIL', error: error.message });
          reject(error);
        }
      }, 10000);
  ...
  printResults() {
    ...
    if (failed === 0) {
      console.log('🎉 All tests passed! Unified bridge server is working correctly.');
    } else {
      console.log('⚠️  Some tests failed. Check the bridge server and extension.');
    }
  }
