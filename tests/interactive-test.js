...
class InteractiveCDPTester {
  ...
  async handleInput(input) {
    ...
    switch (command.toLowerCase()) {
      ...
      case 'title':
        await this.sendCommand('Runtime.evaluate', { expression: 'document.title' });
        break;
...
const scenarios = {
  basic: async (tester) => {
    ...
    await tester.sendCommand('Runtime.evaluate', { expression: 'document.title' });
  },

  advanced: async (tester) => {
    console.log('🧪 Running Advanced Test Scenario\n');
    await tester.sendCommand('Target.createTarget', { url: 'https://httpbin.org/json' });
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    await tester.sendCommand('Runtime.evaluate', { 
      expression: 'fetch("/html").then(r => r.text()).then(html => html.length)' 
    });
    await new Promise(resolve => setTimeout(resolve, 1000));
