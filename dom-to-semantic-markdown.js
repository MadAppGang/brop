const htmlToSMD = ((exports) => {
		...
		element.childNodes.forEach((childElement) => {
			...
			if (overriddenElementProcessing) {
				...
				if (/^h[1-6]$/i.test(elem.tagName)) {
					const level = Number.parseInt(elem.tagName.substring(1));
	...
	function markdownMetaASTToString(nodes, options) {
		...
		if (options.includeMetaData === "extended") {
			if (
				metaNode.content.openGraph &&
				Object.keys(metaNode.content.openGraph).length > 0
			) {
				markdownString += "openGraph:\n";
				markdownString += renderSimpleMetaObject(
					metaNode.content.openGraph,
					"  ",
				);
			}
			if (
				metaNode.content.twitter &&
				Object.keys(metaNode.content.twitter).length > 0
			) {
	...
	const nodeRenderers = {
		...
		bold: (node, options, renderChildren, indentLevel) => {
			const contentString = processNodeContent(
				node.content,
				renderChildren,
				options,
				indentLevel,
		...
		link: (node, options, renderChildren, indentLevel) => {
			...
			if (
				trimmedLinkContent &&
				((Array.isArray(linkNode.content) &&
					linkNode.content.length === 1 &&
					linkNode.content[0].type === "text" &&
					linkNode.content[0].content === trimmedLinkContent) ||
					(typeof linkNode.content === "string" &&
						linkNode.content === trimmedLinkContent) ||
					(!Array.isArray(linkNode.content) &&
						!trimmedLinkContent.includes("<") &&
						!trimmedLinkContent.includes("\n")))
			) {
		...
		list: (node, options, renderChildren, indentLevel) => {
			...
			(listNode.items || []).forEach((item, i) => {
				...
				const itemContentString = renderChildren(
					item.content,
					options,
					indentLevel + 1,
				).trim();
		...
		table: (node, options, renderChildren, indentLevel) => {
			...
			rows.forEach((row, rowIndex) => {
				...
				row.cells.forEach((cell) => {
					if (!cell) return;
					let cellContent = processNodeContent(
						cell.content,
						renderChildren,
						options,
						indentLevel + 1,
		...
		semanticHtml: (node, options, renderChildren, indentLevel) => {
			const htmlNode = node;
			const contentString = renderChildren(
				htmlNode.content,
				options,
				indentLevel,
			);
	...
	function markdownContentASTToStringRecursive(
		...
		const renderChildren = (
			...
			return markdownContentASTToStringRecursive(
				childNodes,
				childOptions,
				childIndent,
			);
		...
		nodes.forEach((node, index) => {
			...
			if (nodeRenderers[node.type]) {
				renderedNodeString = nodeRenderers[node.type]?.(
					node,
					options,
					renderChildren,
					indentLevel,
				...
				renderedNodeString = options.renderCustomNode(
					node,
					options,
					indentLevel,
				);
			...
			if (isCurrentNodeInline) {
				...
				if (markdownString.length > 0 && renderedNodeString.length > 0) {
					...
					const currentStartsWithSpace = /\s/.test(
						firstCharOfCurrentRenderedNode,
					);
					// Punctuation that should not have a leading space if it's the start of the current node.
					const currentStartsWithClingingPunctuation = /^[.,!?;:)]/.test(
						firstCharOfCurrentRenderedNode,
					);
				...
				if (renderedNodeString.length > 0 && index < nodes.length - 1) {
					...
					const isNextNodeBlock =
						BLOCK_NODE_TYPES.has(nextNode.type) &&
						!(nextNode.type === "code" && nextNode.inline);
					if (
						isNextNodeBlock ||
						(nextNode.type === "code" && !nextNode.inline)
					) {
	...
	function markdownASTToString(nodes, options, indentLevel = 0) {
		if (!Array.isArray(nodes)) {
			console.warn(
				"markdownASTToString received non-array input for nodes:",
				nodes,
			);
			return "";
		}
		const metaOutput = markdownMetaASTToString(nodes, options);
		const contentOutput = markdownContentASTToStringRecursive(
			nodes,
			options,
			indentLevel,
		);
	...
	function calculateScore(element) {
		...
		highImpactAttributes.forEach((attr) => {
			if (element.classList.contains(attr) || element.id === attr) {
				score += 10;
				scoreLog.push(
					`High impact attribute found: [${attr}] [${[...element.classList.values()].join(",")}], score increased by 10`,
				);
			}
		});
		// High impact tags
