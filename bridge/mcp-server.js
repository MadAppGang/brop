...
class BROPMCPServer {
  ...
  async initialize() {
    ...
    try {
      ...
      this.log(`BROP MCP Server initialized in ${this.isServerMode ? 'SERVER' : 'RELAY'} mode`);
    } catch (error) {
...
server.tool(
  ...
  async ({ url, tabId }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_navigate', { url, tabId });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  async ({ tabId }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_get_page_content', { tabId });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    tabId: z.number().describe("Tab ID to get content from"),
    format: z.enum(["html", "markdown"]).describe("Output format - html (using Readability) or markdown (semantic conversion)"),
    enableDetailedResponse: z.boolean().optional().describe("Include detailed extraction statistics and metadata"),
  },
  async ({ tabId, format, enableDetailedResponse }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_get_simplified_content', { tabId, format, enableDetailedResponse });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  async ({ script, tabId }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_execute_script', { script, tabId });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  async ({ selector, tabId }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_click_element', { selector, tabId });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  async ({ selector, text, tabId }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_type_text', { selector, text, tabId });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    url: z.string().optional().describe("Optional URL to navigate to in the new page (defaults to about:blank)"),
    active: z.boolean().optional().describe("Whether to make the new tab active (defaults to true)"),
  },
  async ({ url, active }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_create_page', { url, active });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  async ({ tabId }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_close_tab', { tabId });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    windowId: z.number().optional().describe("Optional window ID to filter tabs (if not provided, lists tabs from all windows)"),
    includeContent: z.boolean().optional().describe("Whether to include page content in the response (defaults to false)"),
  },
  async ({ windowId, includeContent }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_list_tabs', { windowId, includeContent });
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  async ({ tabId }) => {
    try {
      const result = await bropServer.executeBROPCommand('brop_activate_tab', { tabId });
      return {
        content: [
          {
            type: "text",
