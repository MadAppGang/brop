...
class UnifiedBridgeServer {
  ...
  handleBropClient(ws, req) {
    ...
    ws.on('error', (error) => {
      this.logger.logError('BROP', 'connection', connectionDisplay, error.message);
    });
  }
  ...
  handleCdpClient(ws, req) {
    ...
    ws.on('error', (error) => {
      this.logger.logError('CDP', 'connection', `${clientId}:${pathname}`, error.message);
      this.cleanupCdpClient(clientId);
    });
  }
  ...
  processBropMessage(client, message) {
    try {
      ...
      const connectionDisplay = clientInfo?.name ? `${clientInfo.id}:${clientInfo.name}` : (clientInfo?.id || 'unknown');
  ...
  processCdpMessage(clientId, message) {
    try {
      ...
      if (!this.extensionClient || this.extensionClient.readyState !== WebSocket.OPEN) {
        ...
        this.logger.logError('CDP', method, `${clientId}:${messageId}`, 'Extension not connected');
        return;
      }
  ...
  sendTargetAttachedEvent(clientId, targetId) {
    ...
    if (clientInfo && clientInfo.ws.readyState === WebSocket.OPEN) {
      ...
      this.logger.logSuccess('CDP', 'event:Target.attachedToTarget', `${clientId}:${targetId}`);
    }
  }
